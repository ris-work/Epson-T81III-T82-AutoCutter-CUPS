...
#define ESC (0x1b)
#define GS  (0x1d)
#define ALWAYS_FEED_BOTTOM_MARGIN_LINES 6
...
static int EndJob(EPTMS_CONFIG_T* p_config, EPTMS_JOB_INFO_T* p_jobInfo, cups_page_header_t* p_header)
{
    ...
    // Always feed 6 lines, then cut
    result = FeedPaper( p_config, p_header, ALWAYS_FEED_BOTTOM_MARGIN_LINES );
    if ( EPTMD_SUCCESS != result ) {
        return 3300; // New error code for feed failure
    }

    // Now issue the cut command
    unsigned char Command[3+4] = { ESC, 'J', 0, GS, 'V', 66, 0 };
    //unsigned char cut_command[3] = { ESC, 'U', 0 }; // Replace with the correct cut command for your printer
    //result = WriteData( cut_command, sizeof(cut_command) );
    result = WriteData( Command, sizeof(Command) );
    if ( EPTMD_SUCCESS != result ) { return 2202; }
    ...
    if ( EPTMD_SUCCESS != result ) {
        return 3301; // New error code for cut failure
    }
